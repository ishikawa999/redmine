# Implementation Gap Analysis: quick-access
...
3. **削除操作の冪等性 — Missing**
   - 現在は所有者スコープでQuickAccessItemが見つからなければ404
   - 他人のQuickAccessItemを非開示にする要件と、削除済みを成功扱いにする要件を両立する識別方法が必要
...
3. **不可視QuickAccessItemの保持 — Existing**
   - 現在の一覧フィルタはQuickAccessItemを削除しないため、権限回復後の再表示を満たす
...
1. **初期ページ描画 — Existing baseline / Constraint**
   - 現在のbase layoutはQuickAccessItemを取得しないため、初期描画にQuickAccessItem queryはない
   - preview内容をサーバー側でbase layoutへ埋め込むと要件に反する
   - ユーザーがhover/focusするまで取得を開始しない必要がある

2. **可視な最新5件の取得 — Research Needed**
   - 現在の`visible_items`はユーザーの全QuickAccessItemを読み込み、Rubyで可視性を評価する
   - 単純に先頭5件を取得してから不可視項目を除くと、閲覧可能な最新5件を満たせない
   - `includes(:target)`だけでは対象のprojectまで十分に先読みできず、型別の認可でN+1余地がある
...
- QuickAccessItem/Controller/HelperをVersionまで拡張
- top menuへ項目を移動
...
### Option B: New isolated quick access navigation component

**Outline**

- QuickAccessItemの基本モデルは再利用
...
- QuickAccessItem CRUDとの状態同期を明示的に設計する必要がある

### Option C: Hybrid

**Outline**

- QuickAccessItemモデル、CRUD Controller、Helper、一覧は既存実装を拡張してVersionと冪等性へ対応
...
- MenuManager、preview、QuickAccessItem CRUD間の状態失効ルールが必要
- Option Aより実装計画がやや複雑
...
- MenuManagerの標準nodeはanchorのHTML属性は持つが`li` wrapper内の任意fragmentを表現しないため、`:quick_access`専用のmenu rendererを設ける。
- responsive.jsはtop menuの`ul`全体をflyoutへ移送する。preview wrapperを同じ`li`内に置き、mobile時は既存toggle可視性を使ってfetchを抑止する。

### Design Decisions

1. **可視読み取りの一般化**: 一覧とpreviewを`QuickAccessItem::VisibleReader`の`limit`差として統一する。previewは最近順キーセット・バッチ走査を行い、固定候補上限による5件欠落を許さない。
2. **Build vs Adopt**: 認可、MenuManager、Stimulus、UJS、responsive flyoutを採用する。汎用dropdownの拡張は既存利用箇所への責務漏れになるため、quick access preview controllerだけを新設する。
3. **冪等解除**: QuickAccessItem IDではなく許可済みtarget identityを入力とし、現在ユーザーscopeの0件削除も成功とする。これにより他ユーザーのQuickAccessItem存在を観測させない。
4. **Progressive enhancement**: 操作は通常HTMLフォーム契約を基礎とし、UJSは同じendpointのJS応答による即時更新だけを追加する。
5. **キャッシュ境界**: preview HTMLはStimulus instance内だけで保持し、QuickAccessItem変更イベントで失効する。サーバー共有cacheは権限変化による漏えいリスクと複雑性のため採用しない。

### Resolved Risks

- 初期描画はmenuの静的wrapperだけをrenderし、QuickAccessItem queryとpreview requestを発生させない。
