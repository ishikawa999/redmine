...
class QuickAccessPreviewTest < ApplicationSystemTestCase
  ...
  [:pointer, :keyboard].each do |input|
    [:loaded, :empty, :error].each do |state|
      define_method("test_#{input}_loading_transitions_to_#{state}") do
        ...
        open_preview(input)
        assert_selector '.quick-access-preview.is-open[data-state="loading"]', text: 'Loading quick access items…'
        assert_equal 1, fetch_count
        page.execute_script('window.releasePinPreviewFetch()')

        assert_selector ".quick-access-preview.is-open[data-state='#{state}']"
        case state
        when :loaded
  ...
  def test_keyboard_internal_focus_escape_and_external_focus_leave
    ...
    assert_selector '.quick-access-preview.is-open[data-state="loaded"] .quick-access-preview-item', count: 3

    parent_link.send_keys(:tab)
    assert_selector '.quick-access-preview-item:first-child a:focus'
    assert_selector '.quick-access-preview.is-open'
    page.send_keys(:tab)
    assert_selector '.quick-access-preview-item:nth-child(2) a:focus'
    assert_selector '.quick-access-preview.is-open'
    page.send_keys(:escape)
    assert_selector '.quick-access-preview:not(.is-open)', visible: :all
    ...
    assert_selector 'a.quick-access:focus'
    assert_selector '.quick-access-preview.is-open[data-state="loaded"]'
    assert_equal 1, fetch_count
    4.times { page.send_keys(:tab) }
    assert_no_selector 'li.quick-access-menu a:focus'
    assert_selector '.quick-access-preview:not(.is-open)', visible: :all
  end
  ...
  def test_real_pin_changes_invalidate_cache_without_fetching_until_reopen
    ...
    preview_menu.hover
    assert_selector '.quick-access-preview-item', count: 3
    find('.quick-access-preview-item a', match: :first).hover
    assert_selector '.quick-access-preview.is-open'
    find('#content').hover
    assert_selector '.quick-access-preview:not(.is-open)', visible: :all
    preview_menu.hover
  ...
  def test_desktop_preview_fetches_once_reuses_cache_and_invalidates_lazily
    ...
    preview_menu.hover
    assert_selector '.quick-access-preview.is-open[data-state="loaded"] .preview-result'
    assert_equal 1, fetch_count
  ...
  def test_desktop_hover_loads_server_fragment_and_preview_link_navigates_to_target
    ...
    assert_selector '.quick-access-preview.is-open[data-state="loaded"] .quick-access-preview-item'
  ...
  def test_focus_escape_and_error_keep_parent_link_available
    ...
    parent_link.send_keys(:tab)
    page.execute_script("document.querySelector('a.quick-access').focus()")
    assert_selector '.quick-access-preview.is-open[data-state="error"]', text: 'Could not load quick access items.'

    page.send_keys(:escape)
    assert_selector '.quick-access-preview:not(.is-open)', visible: :all
    assert parent_link[:href].end_with?('/quick_access')
  end

  def test_loading_transitions_to_empty
    install_fetch_stub('<p class="nodata">You have not added anything to quick access yet.</p>', deferred: true)

    preview_menu.hover
    assert_selector '.quick-access-preview.is-open[data-state="loading"]', text: 'Loading quick access items…'
    page.execute_script('window.resolvePinPreviewFetch()')
    assert_selector '.quick-access-preview.is-open[data-state="empty"]', text: 'You have not added anything to quick access yet.'
  end
  ...
  def test_mobile_navigation_does_not_open_or_fetch_preview
    ...
    preview_menu.hover
    page.execute_script("document.querySelector('.flyout-menu a.quick-access').focus()")
  ...
  def test_mobile_parent_click_navigates_to_list_without_preview_request
    ...
    page.execute_script(<<~JS)
      ...
      window.fetch = function(...args) {
        if (String(args[0]).includes('/quick_access/preview')) {
