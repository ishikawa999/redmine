...
class QuickAccessItemsHelperTest < Redmine::HelperTest
  ...
  test 'status badge reports open and closed state for issues only' do
    ...
    # Versions and wiki pages carry no badge.
  ...
  test 'state labels are available in English and Japanese' do
    expected = {
      en: ['You have not added anything to quick access yet.', 'Could not load quick access items.'],
      ja: ['クイックアクセスに追加されたページはありません。', 'クイックアクセスを取得できませんでした。']
    }

    expected.each do |locale, labels|
      I18n.with_locale(locale) do
        keys = [:label_no_quick_access_items, :label_quick_access_load_error]
